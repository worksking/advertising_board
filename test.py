...
def test(model, label_maps, loader= None):
    ...
    for dirs, _, imgs in os.walk(pjoin("/data1/Adam/advertising_board/datasets/", 'test')):
...
def check_top5_accuracy(model, label_maps, loader=None):
    ...
    print(results)
    with open('./predictions/test2-label-resnet.csv', 'w', newline='') as f:
        title = [['filename', 'label']]
        writer = csv.writer(f)
        writer.writerows(title)
        writer.writerows(results)
...
def main():
    ...
    net = restore("./saved_nets/best_resnet.pkl")
    if  use_cuda:
        net.cuda()
